...
NAME_PREFIX: Final[str] = "rdholmes31"
...
def create_files_from_numeric_range() -> None:
    """Create one file per year for a given inclusive quarter range.
    ...
    for year in range(start_year, end_year + 1):
        ...
        content: str = f"Riley Holmes - Year Automation: {year}\n"
...
def create_files_from_list() -> None:
    """Create files based on a list of sport names.
    ...
    # Define a pet list
    sport_list: list[str] = ["golf", "basketball", "football"]

    # Log my pet list
    LOG.info(f"Sport list ={sport_list}")
    ...
    for sport_name in sport_list:
        ...
        filename: str = f"{NAME_PREFIX}_sport_{sport_name}.txt"
        # Define the path for my new file
        path: Path = ROOT_DIR / filename
        # Define some content to put in the new file
        content: str = f"Here is my sport data for the chosen sport: '{sport_name}'\n"
        # Call the provided helper function to write the file and log it
        write_text_file(path, content)
...
def create_files_using_list_comprehension() -> None:
    ...
    favorite_list: list[str] = [f"{prefix}{name}" for name in sport_list]
    ...
    for favorite in favorite_list:
        ...
        filename: str = f"{NAME_PREFIX}_case_{favorite}.txt"
        # Define the path for my new file
        path: Path = ROOT_DIR / filename
        # Define some content to put in the new file
...
def create_files_periodically() -> None:
    ...
    while i <= count:
        ...
        # Define some content to put in the new file
        content: str = (
            f"Not really sure what I'm doing, but we're on file count:  {i}\n"
        )
        # Call the provided helper function to write the file and log it
        write_text_file(path, content)
